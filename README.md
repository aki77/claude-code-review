...
```
code-review local [--range [<range>]] [--background <text>] [--background-file <path>] [--summary-file <path>] [--debug]
code-review pr <number> [--comment] [--background <text>] [--background-file <path>] [--summary-file <path>] [--debug]
```
...
```bash
pnpm dev -- local [--range [<range>]] [--background <text>] [--background-file <path>] [--summary-file <path>] [--debug]
pnpm dev -- pr <number> [--comment] [--background <text>] [--background-file <path>] [--summary-file <path>] [--debug]
```
...
jobs:
  review:
    ...
    steps:
      ...
      - run: npx @aki77/claude-code-review -- pr ${{ github.event.pull_request.number }} --comment --debug --summary-file "$GITHUB_STEP_SUMMARY"
        continue-on-error: true # confirmed 指摘があると exit 1 になるため
        env:
...
- **終了コード**: confirmed 指摘が 1 件以上あると exit 1 になりステップが失敗扱いになる。
  PR へのコメント投稿・`--summary-file` へのサマリー出力自体は、この exit より前に完了して
  いる。ステップ全体を失敗させたくない場合は `continue-on-error: true` を付ける。
